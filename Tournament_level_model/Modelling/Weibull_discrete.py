...
DiscreteWeibull(2, 2, 2)

with pm.Model() as model:
    
    # beta parameter for each golfer
    beta = pm.Uniform('beta', lower=0.1,upper=100000000)
    
    # Must be between 0 and 1
    q = pm.Uniform('q', lower=0,upper=1)
    
    # Observed scores to follow Discrete Weibull
    golfer_round_score = pm.DiscreteWeibull("golfer_round_score", q=beta,beta=q, observed=observed_total_score)
